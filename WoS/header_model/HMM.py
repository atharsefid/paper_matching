...
import fcntl
###################################################################################################################
cmd_paperids = "select id from papers limit 1000000"
cmd_paper = "select * from papers where id='%s'; "
cmd_author = "select fname, mname, lname from authors where paperid = '%s'"
cmd_medauthor = "select fname, mname, lname from wosauthors where paperid = '%s'"
cmd_medpaper= "select * from wospapers where id = '%s'"
...
def find_match(queue, clf):
    try:
        client = Elasticsearch(timeout = 200, port = 9208)
        csxdb = mysql.connector.connect(user='csx-devel', password='csx-devel', host='csxstaging01', database='citeseerx2', charset='utf8', use_unicode=True)
        CSXcursor = csxdb.cursor( dictionary = True)
        CSXauthorCursor = csxdb.cursor( dictionary = True)
        meddb = mysql.connector.connect(user='csx-devel', password='csx-devel', host='csxstaging01', database='wos2017_12', charset='utf8', use_unicode=True)
    ...
    while(True):
        ...
        try:
            ...
            s = Search(using=client, index="wos2017")
            ...
            for hit in response:
                ...
                if label ==1:
                    with open("match_files1.txt", "a") as g:
