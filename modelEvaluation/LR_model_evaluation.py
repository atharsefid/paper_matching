...
#clf = joblib.load( './models/LR_jaccardModel.pkl')
#print(clf.coef_)
...
f = open('LR_gridsearchCV.txt','w')
for score in scores:
    f.write("# Tuning hyper-parameters for %s \n" % score)
    f.write('\n')
    clf = GridSearchCV(linear_model.LogisticRegression(), param_grid, cv=5,scoring='%s_macro' % score)
...
clf = linear_model.LogisticRegression(**clf.best_params_)
...
clf.fit(pairFeatures,classes)
joblib.dump(clf, 'LR_jaccardModel.pkl')
